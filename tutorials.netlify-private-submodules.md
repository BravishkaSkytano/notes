...
Personally, I like using submodules for my site content so that I don't have to clone all the extra site configuration onto all my devices when all I want to do is edit my posts. Plus, it's easier to setup CMS systems and tools like Obsidian when I don't have to point them to a subfolder three levels down.

At the time of writing this (and probably to this day), I'm using Obsidian to manage my site (also known as my Zettelkasten, or Second Brain). And it's pretty easy to access this on all my devices because I have one repository dedicated to all my markdown files. I can open Obsidian on my laptop, start writing something, sync my work to GitHub, and pick it back up on my phone later, all without worrying about storage space or remembering where in my site configuration my content folder lives. I highly recommend this setup for ease of use, if you can get it configured properly.

### Why Use Private Submodules?

As an added security step, I make my submodules private in case people clone my site, which I keep public in the name of FOSS and the spirit of GitHub. The content that I write is my own, regardless of the theme source files or the software I'm using, and I like to keep it that way. Making my content submodules private means that those who have access to my account, or the organization (if the repo resides in one), can access my site content. And I don't want to join the group of content creators who have had their content stolen without their knowledge and stumble on it later.

Of course, I'm not that popular, so I doubt anyone would *want* to steal my content (at least, not yet), but it's still a good idea to set safeguards in place at the beginning of a project before it becomes a hassle to do later on.

## How To Add A Submodule

 For this tutorial, Netlify requires you to use SSH to authenticate to access private repos. If you have already added the submodule to your repository and you didn't use SSH, continue on and I'll go over how to switch over.

Run the following commands to add a submodule using SSH:

![[tutorials.git.add-submodule#^76869d]]

## Setting Up Netlify

Netlify, by default, supports submodules from GitHub, GitLab, and BitBucket. And, when you're using *private* submodules, you need to give Netlify access to that private repo by using deploy keys.

### Build Configuration

Before you can setup deploy keys with Netlify, you need to have your repository connected. I won't go over the steps here, but follow what you would do for the SSG (Jekyll, Hugo, 11ty, etc.) that you're using. Except you need to preface the build command with an extra step to tell Netlify to initiative and pull submodules.

```
git submodule update --init --recursive && NORMAL_BUILD_COMMAND
```

Make sure you replace NORMAL_BUILD_COMMAND with whatever command your SSG uses, e.g. `hugo` or `bundle exec jekyll build`.
...
You should now be able to build your site!

This does present another issue though, how do I get Netlify to build whenever I push to the content submodule? Remember, Netlify builds when the site repo gets updated, but our content now lives on an entirely separate repo that Netlify isn't monitoring. Is there a way to trigger a build on Netlify from the submodule? Short answer, yes, and here's how.

## Using GitHub Actions To Trigger Netlify Builds

The easiest and best way that I've found to trigger Netlify builds is by using GitHub Actions (since I use GitHub), which are workflow files that run automatically according to certain parameters.[^2]

If you're not using GitHub, GitLab also has their own version of GitHub Actions, but I don't remember what it's called. Go ahead and look on your platform for something called workflows or CI/CD development. You can also lookup how to run workflows on your platform and somebody should be able to help you.

### Adding Personal Access Tokens

Like Netlify, GitHub also needs a key to be able to access your private submodule, but this time we're going to use a **Personal Access Token (PAT)** since both repositories live on GitHub.
...
The basic start would be the following:
